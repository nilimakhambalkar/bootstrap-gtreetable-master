...
(function ($) {
    ...
    function GTreeTable(element, options) {
        ...
        //var columns = [{"id":"state"},{"id":"Country"}];
        
        var columnTemplate = [];
        var columnHeadingTemplate = [];

        if(!options.hasOwnProperty("columns"))
            options.columns = [];
        $.each(options.columns, function(index, value){
            columnTemplate.push("<td>"+value.id+"</td>")
            columnHeadingTemplate.push('<th class="FIELD">TITLE</th>'.replace("TITLE", value.title).replace("FIELD", value.id))
        })
        $(element).find("tr").append(columnHeadingTemplate.join(""));
      ...
      this.templateParts = this.options.templateParts !== undefined ? this.options.templateParts :
            {
                ...
                description: '<span class="' + this.options.classes.description + '"></span>',
    ...
    GTreeTableNode.prototype = {
        ...
        init: function () {
            ...
            this.$description = this.$node.find('.' + this.manager.options.classes.description);
